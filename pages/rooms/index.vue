        ...
        <v-card>
            <v-layout wrap>
                ...
                <v-flex sm6 md6 xs12>
                    <v-toolbar class="no-shadow">
                        <v-spacer/>
                        <v-btn color="success" @click="dialogNew = true;">Добавить</v-btn>
                        <v-btn icon>
        ...
        <v-dialog v-model="dialogEdit" persistent max-width="600px">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-text-field
                            v-model="editRoom.quantity"
                            type="number"
                            label="Количество мест"
                    />
                    <v-select
                            :items="room_types"
                            v-model="editRoom.room_type_id"
                            item-text="name"
                            item-value="id"
                            label="Тип номера"
                    />
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-alert :value="true" type="error" class="mb-3">
                        Не рекомендуется удалять уже использованный номер. На нем могут быть
                        завязаны заявки а так же отчеты. Удаляйте только новые номера,
                        созданные по ошибке.
                    </v-alert>
                    <b>Вы действительно хотите удалить этот номер?</b>
                    <v-text-field
                            v-model="deleteRoom.name"
                            label="Название номера"
                            readonly
                    />
                    <v-text-field
                            v-model="deleteRoom.quantity"
                            type="number"
                            label="Количество мест"
                            readonly
                    />
                    <v-select
                            :items="room_types"
                            v-model="deleteRoom.room_type_id"
                            item-text="name"
                            item-value="id"
                            label="Тип номера"
                            readonly
...
<script>
    export default {
        ...
        methods: {
            ...
            saveEditRoom() {
                this.axios
                    .patch("rooms/" + this.editRoom.id, this.editRoom)
                    .then(response => {
            ...
            saveDeleteRoom() {
                this.axios
                    .delete("rooms/" + this.deleteRoom.id)
                    .then(response => {
