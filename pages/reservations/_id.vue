      ...
      <v-card>
        ...
        <template v-else>
          <v-card-text>
            ...
            >
              <v-tab href="#prepaid"> Liqpay предоплата</v-tab>
              <v-tab href="#paid"> Liqpay оплата</v-tab>
            </v-tabs>
                    ...
                    <v-layout wrap class="card-content ">
                      <v-flex sm6 md6 xs12>
                        ...
                        <v-btn
                          color="success"
                          small
                          @click="liqpay('prepaid',response.orderRoom.sum_prepaid)"
                        >
                        ...
                        <v-btn
                          ...
                          color="success"
                          small
                          @click="liqpayReload('prepaid', response.orderRoom.sum_prepaid )"
                        >
                        ...
                        <v-alert
                          :value="true"
                          type="success"
                          v-if="response.liq_prepaid !== null && response.liq_prepaid.is_prepaid"
                        >
                          Получена предоплата в размере <b>{{response.liq_prepaid.amount}}</b>.
                        </v-alert>
                        ...
                        <v-alert
                          ...
                          v-if="response.liq_prepaid !== null && !response.liq_prepaid.is_prepaid"
                        >
                          Отправлен запрос на предоплату.
                        </v-alert>

                        <v-alert
                          :value="true"
                          type="info"
                          v-if="response.liq_prepaid == null"
                        >
                          Запрос на предоплату еще не был отправлен.
                        </v-alert>
                  ...
                  <v-card-text>
                    <v-layout wrap class="card-content ">
                      <v-flex sm6 md6 xs12>
                        ...
                        />
                        <v-text-field
                          v-model="liqpay_sum_prepaid_without_discount"
                          label="Сумма оплаты со скидкой"
                        />
                        <v-text-field
                          v-model="response.orderRoom.order_num"
                          label="order_id"
                          readonly
                        />
                        <v-btn color="success" small @click="liqpay('paid', liqpay_sum_prepaid_without_discount)">
                        ...
                        <v-btn
                          ...
                          color="success"
                          small
                          @click="liqpayReload('paid', liqpay_sum_prepaid_without_discount);"
                        >
                        ...
                        <v-alert
                          :value="true"
                          type="success"
                          v-if="response.liq_paid !==null && response.liq_paid.is_paid"
                        >
                          Получена оплата в размере <b>{{response.liq_paid.amount}}</b>
                        </v-alert>
                        ...
                        <v-alert
                          ...
                          v-if="response.liq_paid !==null && !response.liq_paid.is_paid"
                        >
                          Отправлен запрос на оплату.
                        </v-alert>

                        <v-alert
                          :value="true"
                          type="info"
                          v-if="response.liq_paid ==null "
                        >
                          Запрос на оплату еще не был отправлен.
                        </v-alert>
  ...
  export default {
    data () {
      return {
        ...
        payment_types: [],
        room_statuses: [],
        liqpay_sum_prepaid_without_discount: null
      }
    },
    mounted () {
      this.init()
    ...
    methods: {
      ...
      saveReservation () {
        ...
        this.axios
          ...
          .then(response => {
            ...
            console.log(response)
            this.$snotify.success('Бронь успешно сохранена!', 'Сохранено')
      ...
      sendMail () {
        ...
        this.axios
          ...
          .then(response => {
            ...
            console.log(response)
            this.$snotify.success('Письмо отослано', 'Успешно')
      ...
      liqpay (order_type, sum_prepaid) {
        this.progress_dialog_liqpay = true

        let request = {
          email: this.response.client.email,
          liqpay_order_id: this.response.orderRoom.order_num + this.response.orderRoom.id + order_type,
          order_room_id: this.response.orderRoom.id,
          sum_prepaid: sum_prepaid,
        ...
        this.axios
          ...
          .then(response => {
            ...
            if (response.data.status == 1) {
              this.$snotify.success(response.data.text, 'Успешно')
              this.dialogLiqpay = false
            } else {
              this.$snotify.error(response.data.text, 'Ошибка')
      ...
      liqpayStatus (order_type) {
        ...
        if (order_type == 'prepaid') {
          ...
          if (this.response.liq_prepaid !== null) {
            // Был отослан инвойс,  в БД хранится запись с liq_pay_order_id
            request.liq_pay_order_id = this.response.liq_prepaid.liq_pay_order_id
          } else {
            // Не был отослан инвойс.
            this.progress_liqpay_prepaid_statuses = false
            this.$snotify.error('Не был отослан инвойс на предоплату.', 'Отправьте запрос на предоплату')
            return
          }
        }
        if (order_type == 'paid') {
          this.progress_liqpay_paid_statuses = true
          if (this.response.liq_paid !== null) {
            // Был отослан инвойс,  в БД хранится запись с liq_pay_order_id
            request.liq_pay_order_id = this.response.liq_paid.liq_pay_order_id
          } else {
            // Не был отослан инвойс.
            this.progress_liqpay_paid_statuses = false
            this.$snotify.error('Не был отослан инвойс на оплату.', 'Отправьте запрос на оплату')
        ...
        this.axios
          ...
          .catch(error => {
            ...
            this.progress_liqpay_prepaid_statuses = false
            this.progress_liqpay_paid_statuses = false
            this.$snotify.error(error.response, 'Произошла ошибка!')

          })
      },
      liqpayReload (order_type, sum_prepaid) {
        this.progress_dialog_liqpay = true
        ...
        const request = {
          ...
          liqpay_order_id: this.response.orderRoom.order_num + this.response.orderRoom.id + order_type,
          order_room_id: this.response.orderRoom.id,
          sum_prepaid: sum_prepaid, // sum_prepaid - универсальная переменная для сумм оплаты и предоплаты.
          order_type: order_type
        }
        console.log(request)
        this.axios
          ...
          .then(response => {
            ...
            if (response.data.status == 1) {
              ...
              this.$snotify.error(response.data.text, 'Внимание')
            }
            this.progress_dialog_liqpay = false
          })
          .catch(error => {
            console.log(error.response)
            this.$snotify.error(error.response, 'Внимание')
            this.progress_dialog_liqpay = false
          })
      }
    }
